...
async def test_start():
    ...
    await pytest.multihead.start()
    assert pytest.multihead.ready, "The Client is not ready"

@pytest.mark.asyncio(loop_scope='session')
async def test_seperate_broadcast_aggregate():
    responses = await pytest.multihead.broadcast(
        "Without any explanation or extra information, just repeat the following: book."
    )
    assert all(head_name in responses for head_name in {"HuggingChat", "Gemini", "Pi"})
    assert all("book" in response.lower() for response in responses.values())

    agg_response = await pytest.multihead.aggregate(
        agents="Gemini",
...
@pytest.mark.asyncio(loop_scope='session')
async def test_broadcast_and_aggregate():
    broadcast_responses, agg_responses = await pytest.multihead.broadcast_and_aggregate(
